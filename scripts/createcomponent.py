...
class JSXFileCreator(FileCreator):
    """Element.jsx file creator"""
    def get_absolute_filename(self) -> Path:
        return self._element.full_path / (self._element.name + ".jsx")

    def _write_file_contents(self):
        self.get_absolute_filename().write_text(
f"""import styles from "./{self._element.name}.module.scss"

export const {self._element.name} = () => (
<div>{self._element.name}</div>
) 
""".strip())


class SCSSFileCreator(FileCreator):
...
class IndexFileCreator(FileCreator):
    """Optional index.js file creator"""
    def get_absolute_filename(self) -> Path:
        return self._element.full_path / "index.js"
    ...
    def _write_file_contents(self):
        ...
        if current_file_contents.strip():
            return
        self.get_absolute_filename().write_text(
            f"""export * from "./{self._element.name}";"""
        )
...
class AskParams:
    ...
    def _ask_element(self, base_folder: BaseFolder) -> str:
        return input(f"Where put it? {base_folder}/").strip()
